    ...
    <aside role="dialog" aria-modal="true" className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="flex h-[90vh] max-h-[700px] w-full max-w-5xl flex-row overflow-hidden rounded-2xl bg-white shadow-2xl">
        <div className="flex flex-col w-full lg:w-3/5 overflow-hidden">
            <header className="flex items-center justify-between bg-gradient-to-r from-violet-600 via-purple-600 to-indigo-600 p-6 text-white shadow-xl">
                <div className="flex items-center gap-4">
                    <div className="flex h-14 w-14 items-center justify-center rounded-full bg-white/20 ring-2 ring-white/40 backdrop-blur-sm">
                      <i className="ri-robot-2-line text-3xl" />
                    </div>
                    <div>
                      <h2 className="text-xl font-bold tracking-tight">Tripy AI Assistant</h2>
                      <p className="text-sm text-violet-100 flex items-center gap-2">
                        <span className="inline-block w-2 h-2 bg-green-400 rounded-full animate-pulse"></span>
                        Real-time booking powered by AI
                      </p>
                    </div>
                </div>
                <button onClick={onClose} aria-label="Close assistant" className="flex h-10 w-10 items-center justify-center rounded-full transition hover:bg-white/20 hover:rotate-90 duration-300">
                  <i className="ri-close-line text-2xl" />
                </button>
            </header>
            <div className="flex-1 overflow-y-auto px-6 py-4 space-y-5 bg-gray-50">
                {!messages.length && <div className="flex h-full flex-col items-center justify-center text-gray-500 px-8">
                  <div className="mb-6 flex h-24 w-24 items-center justify-center rounded-full bg-gradient-to-br from-violet-100 to-purple-100 shadow-lg">
                    <i className="ri-mic-line text-5xl text-violet-600" />
                  </div>
                  <p className="text-xl font-bold text-gray-800 mb-2">Ready to Plan Your Journey!</p>
                  <p className="text-sm text-gray-600 text-center mb-6">I can help you book flights, trains, buses, and hotels with real-time pricing</p>
                  <div className="grid grid-cols-2 gap-3 w-full max-w-sm">
                    <div className="bg-white p-3 rounded-lg shadow-sm border border-gray-200">
                      <i className="ri-flight-takeoff-line text-violet-600 text-xl mb-1"></i>
                      <p className="text-xs font-semibold text-gray-700">Flights</p>
                    </div>
                    <div className="bg-white p-3 rounded-lg shadow-sm border border-gray-200">
                      <i className="ri-hotel-line text-violet-600 text-xl mb-1"></i>
                      <p className="text-xs font-semibold text-gray-700">Hotels</p>
                    </div>
                    <div className="bg-white p-3 rounded-lg shadow-sm border border-gray-200">
                      <i className="ri-train-line text-violet-600 text-xl mb-1"></i>
                      <p className="text-xs font-semibold text-gray-700">Trains</p>
                    </div>
                    <div className="bg-white p-3 rounded-lg shadow-sm border border-gray-200">
                      <i className="ri-bus-line text-violet-600 text-xl mb-1"></i>
                      <p className="text-xs font-semibold text-gray-700">Buses</p>
                    </div>
                  </div>
                </div>}
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 relative">
                      <input type="text" value={fallbackInput} onChange={(e) => setFallbackInput(e.target.value)} onKeyDown={handleKeyDown} disabled={isProcessing} 
                             placeholder={currentAsk ? getPlaceholderForAsk(currentAsk) : "Ask me anything about your trip..."}
                             aria-label="Type your command" className="w-full rounded-xl border-2 border-gray-300 px-5 py-3.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-transparent shadow-sm" />
                      {!fallbackInput && <i className="ri-chat-3-line absolute right-4 top-1/2 -translate-y-1/2 text-gray-400"></i>}
                    </div>
                    <button onClick={() => sendCommand(fallbackInput)} disabled={!fallbackInput.trim() || isProcessing} className="rounded-xl bg-gradient-to-r from-violet-600 to-purple-600 px-7 py-3.5 text-sm font-bold text-white transition hover:from-violet-700 hover:to-purple-700 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg transform hover:scale-105 duration-200">Send</button>
                </div>
            </footer>
        </div>
